    ...
    <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            @endif
            <form id="addAgent" method="post"  action="{{ route('update_delivery_agent') }}" onload="resetForm()" autocomplete="off" enctype="multipart/form-data">
              @csrf
              <input type="hidden"  name="id" class="form-control"  id="id" value="{{ $agentDetails->id ?? '' }}" /> 
               
              <div class="row mx-0">
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 col-12">
                      <div class="profile-image-show mt-3">
                        <a href="javascript:;" class="remove-pro-img {{ $agentDetails->profile_picture!=null && file_exists(public_path('delivery_agent_profile/'.$agentDetails->profile_picture)) ? '':'d-none' }}" id="{{ $agentDetails->id ?? '' }}" style="display:block">
                        ...
                        <img src="{{asset('delivery_agent_profile/'.$agentDetails->profile_picture)}}" class="{{ $agentDetails->profile_picture!=null && file_exists(public_path('delivery_agent_profile/'.$agentDetails->profile_picture)) ? '':'d-none' }}" width="80%" id="profileImage">
                        
                        <div class="thumb_nails {{ $agentDetails->profile_picture!=null && file_exists(public_path('delivery_agent_profile/'.$agentDetails->profile_picture)) ? 'd-none':'' }}">
                          ...
                          <input type="file"  accept="image/*" name="profile_picture" id="profile_picture" class="{{ $agentDetails->profile_picture!=null && file_exists(public_path('delivery_agent_profile/'.$agentDetails->profile_picture)) ? 'd-none':'' }}">  
                      </div>
                </div>
                ...
                <div class="col-sm-6">
                  <div class="form-group">
                    ...
                    <input type="text" name="first_name" class="form-control" id="first_name" maxlength="100" value="{{ $agentDetails->first_name ?? '' }}">
                  </div>
                </div>
                ...
                <div class="col-sm-6">
                  <div class="form-group">
                    ...
                    <input type="text" name="last_name" class="form-control" id="last_name" maxlength="100" value="{{ $agentDetails->last_name ?? '' }}">                 
                  </div>
                </div>
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                      ...
                      <input type="text" name="email" class="form-control" id="email" maxlength="100" value="{{ $agentDetails->email ?? '' }}" readonly>                 
                ...
                <div class="col-6">
                  <div class="form-group">
                    ...
                    <br>
                    <input type="tel"  name="phone_number" class="form-control"  id="txtPhone"  autocomplete="false" value="{{ $agentDetails->phone_number ?? '' }}" />
                    <input type="hidden"  name="country_code" class="form-control"  id="country_code" value="{{ $agentDetails->country_code ?? '' }}" /> 
                  </div>
                </div>
              ...
              <div class="card-footer text-left">
                <button type="text" class="btn btn-primary common_btn addagent_btn">{{ __('adminlte::adminlte.update') }}</button>
              </div>
            </form>
          </div>
        </div>
...
<script type="text/javascript">
  $(function () {
      var code = "+"+"{{ $agentDetails->country_code}}{{ $agentDetails->phone_number}}"; // Assigning value from model.
      $('#txtPhone').val(code);
  ...
  // just ascii letters
          ...
          $(".remove-pro-img").click(function(evt){
                    ...
                    if (typeof attr !== 'undefined' && attr !== false) {
                       $.ajax({
                             url:"{{ route('deliveryAgent.delete.profileImage') }}",
                             method:"POST",
                             data:{
                                'delivery_agent_id':attr,
                                "_token": "{{csrf_token()}}"
                             },
