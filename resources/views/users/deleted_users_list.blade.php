          ...
          <table style="width:100%" id="users-list" class="table table-bordered table-hover yajra-datatable">
            ...
            <tbody>
             ...
             <tr> 
              ...
              @if($data->homeowner == '1')
              <td> {{ $data->homeowner_first_name  ?? ''}} {{ $data->homeowner_last_name  ?? ''}}</td>
              <td> {{ $data->homeowner_email  ?? ''}}</td>
              @else
              <td> {{ $data->nominator_first_name  ?? ''}} {{ $data->nominator_last_name  ?? ''}}</td>
              <td> {{ $data->nominator_email  ?? ''}}</td>
              @endif
...
if (willDelete) {
  $.ajax({
    url: "{{ route('permanent_delete_user_u') }}",
    type: 'post',
    data: {
      id: id
    },
